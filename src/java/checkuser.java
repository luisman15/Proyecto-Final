        ...
        try (PrintWriter out = response.getWriter()) {            
            ...
            ResultSet rs = base.consultaSQL("select * from usuarios where usuario =" + "'" + request.getParameter("inputEmail") + "'" + " and clave = " + "'" + request.getParameter("inputPassword") + "'");
                ...
                while(rs.next()){
                    ...
                    out.println("Tu usuario es: " + rs.getString("usuario") + "." + "<BR>");
                  
                    out.println("Bienvenido" + " " + rs.getString("nombreyapellido") + " " + "<BR>");
                }
                
                if (rs.first()==false){
                out.println("<h1>Ingreso incorrecto.</h1>");
                out.println("<h2>No hay usuarios que coincidan con la búsqueda.</h2>");
                out.println("Proyecto: " + request.getContextPath() + ". <BR>");
                out.println("Usuario: " + request.getParameter("inputEmail")+ ".");
